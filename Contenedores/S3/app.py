...
if not S3_BUCKET:
    raise RuntimeError("Falta la variable de entorno S3_BUCKET con el nombre del bucket a usar.")

s3 = boto3.client('s3', region_name=AWS_REGION or None)


def clave_objeto(timestamp, origen):
    # El timestamp con ancho fijo hace que el orden lexicografico de S3
    # coincida con el orden cronologico, sin tener que ordenar en el cliente.
    return f"{PREFIX}{timestamp:020.6f}_{origen}.json"
...
def listar_claves():
    ...
    while True:
        ...
        claves.extend(obj["Key"] for obj in resp.get("Contents", []))
...
<div class="card">
    <div class="top-bar">
        <div>
            <h1>&gt; CYBERDECK_IOT</h1>
            <p class="subtitle">monitor de senal :: s3 (boto3)</p>
        </div>
    ...
    <div class="footer-row">
        ...
        <span>almacen: <code>s3</code></span>
    </div>
</div>
...
def datos_iot():
    if request.method == 'POST':
        ...
        timestamp = time.time()
        registro = {"timestamp": timestamp, "origen": origen, "payload": data}

        s3.put_object(
            Bucket=S3_BUCKET,
            Key=clave_objeto(timestamp, origen),
            Body=json.dumps(registro).encode('utf-8'),
            ContentType='application/json',
        )

        app.logger.info(f"[{origen.upper()}] Dato guardado en S3: {data}")
        return jsonify({"status": "recibido", "data": registro}), 201
    ...
    resultado = []
    for clave in listar_claves():
        cuerpo = s3.get_object(Bucket=S3_BUCKET, Key=clave)["Body"].read()
        resultado.append(json.loads(cuerpo))
