...
DB_PASSWORD = os.environ.get('DB_PASSWORD', 'iot_pass')
DB_NAME = os.environ.get('DB_NAME', 'iot_local')
...
def init_db():
    ...
    for intento in range(1, intentos_maximos + 1):
        try:
            ...
            app.logger.warning(f"[init_db] MariaDB no lista (intento {intento}/{intentos_maximos}): {e}")
            time.sleep(3)
...
<div class="card">
    <div class="top-bar">
        <div>
            <h1>&gt; CYBERDECK_IOT</h1>
            <p class="subtitle">monitor de senal :: motor local mariadb</p>
        </div>
    ...
    <div class="footer-row">
        ...
        <span>motor: <code>mariadb</code></span>
    </div>
</div>
...
def datos_iot():
    if request.method == 'POST':
        ...
        app.logger.info(f"[{origen.upper()}] Dato guardado en MariaDB: {data}")
