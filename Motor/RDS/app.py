...
if not DB_HOST or not DB_USER or not DB_NAME:
    raise RuntimeError(
        "Faltan variables de entorno de RDS. Define DB_HOST (endpoint de RDS), "
        "DB_USER, DB_PASSWORD y DB_NAME antes de iniciar el contenedor."
    )
...
def init_db():
    ...
    for intento in range(1, intentos_maximos + 1):
        try:
            ...
            app.logger.warning(f"[init_db] RDS no responde (intento {intento}/{intentos_maximos}): {e}")
            time.sleep(3)
    raise RuntimeError("No se pudo conectar a la instancia RDS tras varios intentos")
...
<div class="card">
    <div class="top-bar">
        <div>
            <h1>&gt; CYBERDECK_IOT</h1>
            <p class="subtitle">monitor de senal :: motor externo (rds)</p>
        </div>
    ...
    <div class="footer-row">
        ...
        <span>motor: <code>rds mysql</code></span>
    </div>
</div>
...
def datos_iot():
    if request.method == 'POST':
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO lecturas (timestamp, origen, payload) VALUES (%s, %s, %s)",
        ...
        app.logger.info(f"[{origen.upper()}] Dato guardado en RDS: {data}")
    ...
    cur.execute("SELECT timestamp, origen, payload FROM lecturas ORDER BY id ASC")
    filas = cur.fetchall()
    cur.close()
    conn.close()
